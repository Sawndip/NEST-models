...
class LaplacianParameter: public nest::Parameter
{
    public:
        LaplacianParameter(const DictionaryDatum& d):
            Parameter(d),
            a_(3.0),
            b_(3.0),
            r_(3.0)
        ...
        {
            double xx;
            xx=disp[0]*disp[0]+disp[1]*disp[1];
            return (1.0 + a_) * std::exp(-xx / (2.0 * r_ * r_))
                - a_ * std::exp(-xx / (2.0 * b_ * b_ * r_ * r_));
        }

        nest::Parameter * clone() const
        {
            return new LaplacianParameter(*this);
        }
