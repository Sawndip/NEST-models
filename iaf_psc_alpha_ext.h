...
#ifndef IAF_PSC_ALPHA_H_EXT
#define IAF_PSC_ALPHA_H_EXT
...
/* BeginDocumentation
Name: iaf_psc_alpha_ext - Leaky integrate-and-fire neuron model.

Description:

  iaf_psc_alpha_ext is an implementation of a leaky integrate-and-fire model
  ...
  The iaf_psc_alpha_ext is the standard model used to check the consistency
...
Remarks:
  ...
  optimization levels. A future version of iaf_psc_alpha_ext will probably
...
namespace mynest
{
   ...
   */
  class iaf_psc_alpha_ext : public Archiving_Node
  {
    
  public:
    
    iaf_psc_alpha_ext();
    iaf_psc_alpha_ext(const iaf_psc_alpha_ext&);
  ...
  private:
    ...
    friend class RecordablesMap<iaf_psc_alpha_ext>;
    friend class UniversalDataLogger<iaf_psc_alpha_ext>;
    ...
    struct Parameters_ {
      ...
      /** Time constant (rising and falling) of excitatory synaptic current in ms. */
      double_t tau_ex_r_;
      double_t tau_ex_f_;

      /** Time constant (rising and falling) of inhibitory synaptic current in ms. */
      double_t tau_in_r_;
      double_t tau_in_f_;
    ...
    struct Buffers_ {
      ...
      Buffers_(iaf_psc_alpha_ext&);
      Buffers_(const Buffers_&, iaf_psc_alpha_ext&);
      ...
      UniversalDataLogger<iaf_psc_alpha_ext> logger_;
    ...
    /**
     * @defgroup iaf_psc_alpha_ext_data
    ...
    static RecordablesMap<iaf_psc_alpha_ext> recordablesMap_;
  };

  inline
  port iaf_psc_alpha_ext::check_connection(Connection& c, port receptor_type)
  {
    SpikeEvent e;
    e.set_sender(*this);
    c.check_event(e);
  ...
  inline
  port iaf_psc_alpha_ext::connect_sender(SpikeEvent&, port receptor_type)
  {
    if (receptor_type != 0)
  ...
  inline
  port iaf_psc_alpha_ext::connect_sender(CurrentEvent&, port receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
    return 0;
  ...
  inline
  port iaf_psc_alpha_ext::connect_sender(DataLoggingRequest& dlr, port receptor_type)
  {
    if (receptor_type != 0)
      throw UnknownReceptorType(receptor_type, get_name());
  ...
  inline
  void iaf_psc_alpha_ext::get_status(DictionaryDatum &d) const
  {
    P_.get(d);
    S_.get(d, P_);
  ...
  inline
  void iaf_psc_alpha_ext::set_status(const DictionaryDatum &d)
  {
