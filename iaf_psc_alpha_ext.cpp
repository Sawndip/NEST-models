...
nest::RecordablesMap<mynest::iaf_psc_alpha_ext> mynest::iaf_psc_alpha_ext::recordablesMap_;
...
namespace nest
{
   ...
   */
  template <>
  void RecordablesMap<mynest::iaf_psc_alpha_ext>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, &mynest::iaf_psc_alpha_ext::get_V_m_);
    insert_(names::weighted_spikes_ex, &mynest::iaf_psc_alpha_ext::get_weighted_spikes_ex_);
    insert_(names::weighted_spikes_in, &mynest::iaf_psc_alpha_ext::get_weighted_spikes_in_);
    insert_(names::input_currents_ex, &mynest::iaf_psc_alpha_ext::get_input_currents_ex_);
    insert_(names::input_currents_in, &mynest::iaf_psc_alpha_ext::get_input_currents_in_);
  }
}
...
namespace mynest
{
  ...
  mynest::iaf_psc_alpha_ext::Parameters_::Parameters_()
    : Tau_       ( 10.0    ),  // ms
      C_         (250.0    ),  // pF
      TauR_      (  2.0    ),  // ms
      U0_        (-70.0    ),  // mV
  ...
  mynest::iaf_psc_alpha_ext::State_::State_()
    : y0_   (0.0),
      y1_ex_(0.0),
      y2_ex_(0.0),
      y1_in_(0.0),
  ...
  void mynest::iaf_psc_alpha_ext::Parameters_::get(DictionaryDatum &d) const
  {
    ...
    def<double>(d, "tau_syn_ex_rise", tau_ex_r_);
    def<double>(d, "tau_syn_ex_fall", tau_ex_f_);
    def<double>(d, "tau_syn_in_rise", tau_in_r_);
    def<double>(d, "tau_syn_in_fall", tau_in_f_);
  }

  double mynest::iaf_psc_alpha_ext::Parameters_::set(const DictionaryDatum& d)
  {
    ...
    updateValue<double>(d, "tau_syn_ex_rise", tau_ex_r_);
    updateValue<double>(d, "tau_syn_ex_fall", tau_ex_f_);
    updateValue<double>(d, "tau_syn_in_rise", tau_in_r_);
    updateValue<double>(d, "tau_syn_in_fall", tau_in_f_);
    ...
    if (tau_ex_r_ <= 0.0 || tau_in_r_ <= 0.0 
            || tau_ex_f_ <= 0.0 || tau_in_f_ <= 0.0 )
      throw BadProperty("All synaptic time constants must be > 0.");

    if ( Tau_ == tau_ex_r_ || Tau_ == tau_in_r_ )
  ...
  void mynest::iaf_psc_alpha_ext::State_::get(DictionaryDatum &d, const Parameters_& p) const
  {
    def<double>(d, names::V_m, y3_ + p.U0_); // Membrane potential
  }

  void mynest::iaf_psc_alpha_ext::State_::set(const DictionaryDatum& d, const Parameters_& p, double delta_EL)
  {
  ...
  mynest::iaf_psc_alpha_ext::Buffers_::Buffers_(iaf_psc_alpha_ext& n)
    : logger_(n)
  {}

  mynest::iaf_psc_alpha_ext::Buffers_::Buffers_(const Buffers_ &, iaf_psc_alpha_ext& n)
    : logger_(n)
  {}
  ...
  mynest::iaf_psc_alpha_ext::iaf_psc_alpha_ext()
    : Archiving_Node(),
      P_(),
      S_(),
      B_(*this)
  ...
  mynest::iaf_psc_alpha_ext::iaf_psc_alpha_ext(const iaf_psc_alpha_ext& n)
    : Archiving_Node(n),
      P_(n.P_),
      S_(n.S_),
      B_(n.B_, *this)
  ...
  void mynest::iaf_psc_alpha_ext::init_state_(const Node& proto)
  {
    const iaf_psc_alpha_ext& pr = downcast<iaf_psc_alpha_ext>(proto);
    S_ = pr.S_;
  }

  void mynest::iaf_psc_alpha_ext::init_buffers_()
  {
  ...
  void mynest::iaf_psc_alpha_ext::calibrate()
  {
    ...
    V_.P11_ex_ = std::exp(-h/P_.tau_ex_r_);
    V_.P22_ex_ = std::exp(-h/P_.tau_ex_f_);
    V_.P21_ex_ = 1.0 - V_.P22_ex_;

    V_.P11_in_ = std::exp(-h/P_.tau_in_r_);
    V_.P22_in_ = std::exp(-h/P_.tau_in_f_);
    V_.P21_in_ = 1.0 - V_.P22_in_;

    V_.P33_ = numerics::expm1(-h/P_.Tau_);
    V_.P30_ = (1.0 - V_.P33_) / P_.C_;
    ...
    // a double_t in ms. The grid based iaf_psc_alpha_ext can only handle refractory
    ...
    //
    // The definition of the refractory period of the iaf_psc_alpha_ext is consistent
    // the one of iaf_psc_alpha_ext_ps.
    //
  ...
  void mynest::iaf_psc_alpha_ext::update(Time const & origin, const long_t from, const long_t to)
  {
    ...
    for ( long_t lag = from ; lag < to ; ++lag )
    {
      if ( S_.r_ == 0 )
      {
        ...
        S_.y3_ = V_.P30_*(S_.y0_ + P_.I_e_+ S_.y2_ex_ + S_.y2_in_)
                 + V_.P33_ * S_.y3_;
      ...
      S_.y1_ex_ += V_.weighted_spikes_ex_;
      ...
      S_.y1_in_ += V_.weighted_spikes_in_;
  ...
  void mynest::iaf_psc_alpha_ext::handle(SpikeEvent& e)
  {
  ...
  void mynest::iaf_psc_alpha_ext::handle(CurrentEvent& e)
  {
    assert(e.get_delay() > 0);
  ...
  void mynest::iaf_psc_alpha_ext::handle(DataLoggingRequest& e)
  {
    B_.logger_.handle(e);
  }
