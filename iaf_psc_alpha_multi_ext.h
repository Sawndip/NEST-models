...
#ifndef IAF_PSC_ALPHA_MULTI_EXT_H
#define IAF_PSC_ALPHA_MULTI_EXT_H
...
#include "recordables_map.h"
  ...
  /* BeginDocumentation
Name: iaf_psc_alpha_multi_ext - Leaky integrate-and-fire neuron model with multiple ports.

Description:

  iaf_psc_alpha_multi_ext is a direct extension of iaf_psc_alpha.
...
namespace mynest
{
   ...
   */
  class iaf_psc_alpha_multi_ext : public Archiving_Node
  {
    
  public:
    
    iaf_psc_alpha_multi_ext();
    iaf_psc_alpha_multi_ext(const iaf_psc_alpha_multi_ext&);
  ...
  private:
    ...
    friend class RecordablesMap<iaf_psc_alpha_multi_ext>;
    friend class UniversalDataLogger<iaf_psc_alpha_multi_ext>;
     ...
     */
    struct Buffers_ {
      Buffers_(iaf_psc_alpha_multi_ext &);
      Buffers_(const Buffers_ &, iaf_psc_alpha_multi_ext &);
      ...
      UniversalDataLogger<iaf_psc_alpha_multi_ext> logger_;
    };
    ...
    /**
     * @defgroup iaf_psc_alpha_multi_ext_data
    ...
    static RecordablesMap<iaf_psc_alpha_multi_ext> recordablesMap_;
  };

inline
port iaf_psc_alpha_multi_ext::check_connection(Connection& c, port receptor_type)
{
  SpikeEvent e;
  e.set_sender(*this);
  c.check_event(e);
...
inline
port iaf_psc_alpha_multi_ext::connect_sender(CurrentEvent&, port receptor_type)
{
  if (receptor_type != 0)
...
inline
port iaf_psc_alpha_multi_ext::connect_sender(DataLoggingRequest& dlr, 
                                   port receptor_type)
{
  if (receptor_type != 0)
    throw UnknownReceptorType(receptor_type, get_name());
...
inline
void iaf_psc_alpha_multi_ext::get_status(DictionaryDatum &d) const
{
  P_.get(d);
  S_.get(d, P_);
...
inline
void iaf_psc_alpha_multi_ext::set_status(const DictionaryDatum &d)
{
