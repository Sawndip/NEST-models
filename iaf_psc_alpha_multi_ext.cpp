...
nest::RecordablesMap<mynest::iaf_psc_alpha_multi_ext> mynest::iaf_psc_alpha_multi_ext::recordablesMap_;
...
namespace nest
{
  ...
  template <>
  void RecordablesMap<mynest::iaf_psc_alpha_multi_ext>::create()
  {
    // use standard names whereever you can for consistency!
    insert_(names::V_m, &mynest::iaf_psc_alpha_multi_ext::get_V_m_);
    insert_(names::currents, &mynest::iaf_psc_alpha_multi_ext::get_current_);
  }
}
...
namespace mynest
{
mynest::iaf_psc_alpha_multi_ext::Parameters_::Parameters_()
  : Tau_                 ( 10.0    ),  // ms
    C_                   (250.0    ),  // pF
    TauR_                (  2.0    ),  // ms
    U0_                  (-70.0    ),  // mV
...
mynest::iaf_psc_alpha_multi_ext::State_::State_()
  : y0_   (0.0),  
    y3_   (0.0),
    r_    (0)
{
...
void mynest::iaf_psc_alpha_multi_ext::Parameters_::get(DictionaryDatum &d) const
{
...
double mynest::iaf_psc_alpha_multi_ext::Parameters_::set(const DictionaryDatum& d)
{
...
void mynest::iaf_psc_alpha_multi_ext::State_::get(DictionaryDatum& d, const Parameters_& p) const
{
  def<double>(d, names::V_m, y3_ + p.U0_); // Membrane potential
}

void mynest::iaf_psc_alpha_multi_ext::State_::set(const DictionaryDatum& d, const Parameters_& p, const double delta_EL)
{
...
mynest::iaf_psc_alpha_multi_ext::Buffers_::Buffers_(iaf_psc_alpha_multi_ext& n)
  : logger_(n)
{}

mynest::iaf_psc_alpha_multi_ext::Buffers_::Buffers_(const Buffers_ &, iaf_psc_alpha_multi_ext& n)
  : logger_(n)
{}
...
mynest::iaf_psc_alpha_multi_ext::iaf_psc_alpha_multi_ext()
  : Archiving_Node(), 
    P_(), 
    S_(),
    B_(*this)
...
mynest::iaf_psc_alpha_multi_ext::iaf_psc_alpha_multi_ext(const iaf_psc_alpha_multi_ext& n)
  : Archiving_Node(n), 
    P_(n.P_), 
    S_(n.S_),
    B_(n.B_, *this)
...
void mynest::iaf_psc_alpha_multi_ext::init_state_(const Node& proto)
{
  const iaf_psc_alpha_multi_ext& pr = downcast<iaf_psc_alpha_multi_ext>(proto);
  S_ = pr.S_;
}

void mynest::iaf_psc_alpha_multi_ext::init_buffers_()
{
...
void mynest::iaf_psc_alpha_multi_ext::calibrate()
{
...
void mynest::iaf_psc_alpha_multi_ext::update(Time const& origin, const long_t from, const long_t to)
{
  ...
  for ( long_t lag = from ; lag < to ; ++lag )
  {
    if ( S_.r_ == 0 )
    {
      ...
      S_.y3_ = V_.P30_ * S_.current_ + V_.P33_*S_.y3_;
...
port mynest::iaf_psc_alpha_multi_ext::connect_sender(SpikeEvent&, port receptor_type)
{
...
void mynest::iaf_psc_alpha_multi_ext::handle(SpikeEvent& e)
{
...
void mynest::iaf_psc_alpha_multi_ext::handle(CurrentEvent& e)
{
  assert(e.get_delay() > 0);
...
void mynest::iaf_psc_alpha_multi_ext::handle(DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}
