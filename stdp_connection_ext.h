...
#ifndef STDP_CONNECTION_EXT_H
#define STDP_CONNECTION_EXT_H

/* BeginDocumentation
  Name: stdp_synapse_ext - Extended Synapse type for spike-timing dependent
   plasticity.

  Description:
   stdp_synapse_ext is an extension of stdp_synapse with two more parameters.
  ...
  Parameters:
   ...
   EmitSpk    bool   - Determine whether to emit spikes or not
   Esyn       double - Multiplication to w when sending spikes
   LearnEn    bool   - Determine whether enable learning or not
   Gpre       double - None-STDP pre-synaptic learning factor
   Gpost      double - None-STDP post-synaptic learning factor
...
namespace mynest
{
  class STDPConnectionExt : public ConnectionHetWD
  {
   ...
   */
  STDPConnectionExt(const STDPConnectionExt &);
...
inline
double_t STDPConnectionExt::facilitate_(double_t w, double_t kplus)
{
  double_t norm_w = (w / Wmax_) + (lambda_ * std::pow(1.0 - (w/Wmax_), mu_plus_) * kplus);
  norm_w -= lambda_ * Gpost_ / Wmax_;
  if (norm_w < 0.0)
      return 0.0;
  else
...
inline 
double_t STDPConnectionExt::depress_(double_t w, double_t kminus)
{
  double_t norm_w = (w / Wmax_) - (alpha_ * lambda_ * std::pow(w/Wmax_, mu_minus_) * kminus);
  norm_w += lambda_ * Gpre_ / Wmax_;
  if (norm_w > 1.0)
    return Wmax_;
  else
...
inline 
void STDPConnectionExt::check_connection(Node & s, Node & r, rport receptor_type, double_t t_lastspike)
{
 ...
 */
inline
void STDPConnectionExt::send(Event& e, double_t t_lastspike, const CommonSynapseProperties &)
{
...
#endif // of #ifndef STDP_CONNECTION_EXT_H
